...
def _format_filename(filename, args):
...
def format(names, args) -> int:
    ...
    for name in names:
        ...
        if os.path.isfile(name):
            _format_filename(name, args)
            continue
        if os.path.isdir(name):
            _format_dirname(name, args)
            continue
    return 0
